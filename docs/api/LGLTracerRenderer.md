...
**v1.1 this option has no effect temporarily**
...
Fixed the total number of screen splits when tileRender is enabled. Setting a larger number for this item will help improve performance (such as 16).
...
Update the Material used by the Mesh.

```javascript
sphereMesh.material.color = new THREE.Color(1, 0, 0);
// Or replace with new: sphereMesh.material = new DisneyMaterial();
...
Update Mesh's Transform properties (position, rotation, scale) and visible properties.
...
### .setTileSlicesNumber(value: Number)
* Default: `0`
